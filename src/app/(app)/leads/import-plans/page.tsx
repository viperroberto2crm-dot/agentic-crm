...
function downloadTemplate() {
  ...
  const readme = XLSX.utils.aoa_to_sheet([
    ...
    [""],
    ["assigned_rep: nombre del rep que atiende a este paciente (ej. 'Andreina', 'Moe', 'Honorina')."],
    ["  El sistema busca el usuario activo cuyo nombre contenga ese texto. Crea primero los reps en"],
    ["  Settings → Users si no existen. Si no encuentra match, el lead queda asignado a ti y el texto"],
    ["  se guarda en notas para que no se pierda."],
    [""],
    ...
    ["plan_notes: notas libres (observaciones del paciente, contexto, etc.)."],
    [""],
    ["HOJA PAYMENTS (opcional, solo si tienes abonos cobrados ya):"],
    ["  ref_id: debe matchear el ref_id del plan en la hoja Plans."],
    ["  amount: monto del pago en dólares (ej. 75)."],
    ["  date: fecha del pago. Formato MM/DD/YYYY."],
    ...
    reader.onload = (ev) => {
      try {
        ...
        const parsedPlans: PlanImportRow[] = plansRaw.map((r) => ({
          ...
          assigned_rep: toNullableString(r.assigned_rep),
